...
def create_agencia(agencia: AgenciaCreate, db: Session = Depends(get_db)):
    try:
        created = agencia_service.create_agencia(db, agencia)
...
def get_agencia(agencia_id: UUID, db: Session = Depends(get_db)):
    try:
...
def update_agencia(
    ...
    try:
        ...
        agencia = agencia_service.update_agencia(db, str(agencia_id), data)
        return agencia_service.to_response(agencia)
    except Exception as exc:
        handle_agencia_error(exc)
...
def reativar_agencia(
    ...
    try:
        agencia = agencia_service.reativar_agencia(db, str(agencia_id), actor_usuario_id=actor_usuario_id)
        return agencia_service.to_response(agencia)
    except Exception as exc:
        handle_agencia_error(exc)
