...
def create_workflow_modelo(
    ...
    current_user: Usuario = Depends(require_permissao("workflows.criar")),
...
def list_workflow_modelos(
    ...
    current_user: Usuario = Depends(require_permissao("workflows.visualizar")),
    db: Session = Depends(get_db),
):
...
def list_diretorio(
    ...
    # workflow ao criar tarefa não é administrar Workflow (isso continua exigindo
    # "workflows.criar"/"workflows.editar"/"workflows.arquivar" — ver Fase 2G.10B — em todas
    # as outras rotas deste router). Só ativo: não há referência histórica a resolver aqui,
    # diferente do /diretorio de Departamento/Cliente.
...
def update_workflow_modelo(
    ...
    current_user: Usuario = Depends(require_permissao("workflows.editar")),
    db: Session = Depends(get_db),
):
    try:
...
def arquivar_workflow_modelo(
    ...
    current_user: Usuario = Depends(require_permissao("workflows.arquivar")),
    db: Session = Depends(get_db),
):
    try:
        existente = workflow_modelo_service.get_workflow_modelo(db, str(workflow_modelo_id))
