...
from app.schemas.cargo import CargoUpdate
...
def cargo(empresa_id: str, **overrides):
    data = {
        "id": str(uuid4()),
        "empresa_id": empresa_id,
...
def test_cargo_persists_required_fields(session_factory):
    emp = persist(session_factory, empresa())
    created = persist(session_factory, cargo(emp.id, codigo_interno="CG-001", nome="Diretor de Arte"))

    assert created.id
    assert created.empresa_id == emp.id
    assert created.codigo_interno == "CG-001"
    assert created.nome == "Diretor de Arte"
    assert created.status == "ativa"
    assert Cargo.__table__.c.created_at.type.timezone is True
    assert Cargo.__table__.c.updated_at.type.timezone is True


def test_model_is_registered_in_base_metadata():
    assert "cargos" in Base.metadata.tables


def test_expected_columns_exist():
    assert set(Cargo.__table__.columns.keys()) == {
        "id",
        "empresa_id",
        "codigo_interno",
        "nome",
...
def test_empresa_id_is_required(session_factory):
    with pytest.raises(IntegrityError):
        persist(session_factory, cargo(None))


def test_empresa_id_must_reference_existing_empresa(session_factory):
    with pytest.raises(IntegrityError):
        persist(session_factory, cargo(str(uuid4())))
...
def test_inativado_por_usuario_id_must_reference_existing_usuario(session_factory):
    ...
    with pytest.raises(IntegrityError):
        persist(session_factory, cargo(emp.id, inativado_por_usuario_id=str(uuid4())))
...
def test_codigo_interno_unique_per_empresa(session_factory):
    emp = persist(session_factory, empresa())
    persist(session_factory, cargo(emp.id, codigo_interno="CG-DUP"))
...
def test_codigo_interno_can_repeat_in_different_empresas(session_factory):
    ...
    first = persist(session_factory, cargo(emp_a.id, codigo_interno="CG-001"))
    second = persist(session_factory, cargo(emp_b.id, codigo_interno="CG-001"))
...
def test_nome_unique_per_empresa(session_factory):
    emp = persist(session_factory, empresa())
    persist(session_factory, cargo(emp.id, nome="Atendimento"))
...
def test_nome_can_repeat_in_different_empresas(session_factory):
    ...
    first = persist(session_factory, cargo(emp_a.id, nome="Redator"))
    second = persist(session_factory, cargo(emp_b.id, nome="Redator"))
...
def test_nome_empty_is_rejected(session_factory):
    ...
    with pytest.raises(IntegrityError):
        persist(session_factory, cargo(emp.id, nome="   "))
...
def test_codigo_interno_empty_is_rejected(session_factory):
    ...
    with pytest.raises(IntegrityError):
        persist(session_factory, cargo(emp.id, codigo_interno="   "))


@pytest.mark.parametrize("status", ["ativa", "inativa", "arquivada"])
def test_allowed_status_values(session_factory, status):
    emp = persist(session_factory, empresa())
    created = persist(session_factory, cargo(emp.id, status=status))
...
def test_invalid_status_is_rejected(session_factory):
    ...
    with pytest.raises(IntegrityError):
        persist(session_factory, cargo(emp.id, status="bloqueada"))


def test_descricao_can_be_null(session_factory):
    emp = persist(session_factory, empresa())
    created = persist(session_factory, cargo(emp.id, descricao=None))
...
def test_cargo_update_rejects_forbidden_fields():
    forbidden = [
        "empresaId",
        "status",
        "createdAt",
    ...
    for field in forbidden:
        with pytest.raises(ValidationError):
            CargoUpdate.model_validate({field: "valor"})
