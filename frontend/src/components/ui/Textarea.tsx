...
export function Textarea({ label, className, density = "default", ...rest }: TextareaProps) {
  return (
    <label className="block text-sm">
      ...
      <textarea
        rows={3}
        className={`w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 ${fieldTextClassNames[density]} text-zinc-900 outline-none transition focus:border-zinc-400 ${className ?? ""}`}
        {...rest}
      />
    </label>
  );
