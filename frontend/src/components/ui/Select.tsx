...
export function Select({
  ...
  return (
    <label className="block text-sm">
      ...
      <select
        className={`w-full rounded-xl border border-zinc-200 bg-white ${fieldPaddingClassNames[density]} ${fieldTextClassNames[density]} text-zinc-900 outline-none transition focus:border-zinc-400 ${className ?? ""}`}
        {...rest}
      >
        {options.map((option) => (
